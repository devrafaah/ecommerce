...
import { Order, OrderQuery, OrderStatus } from "../models/order.model.js";
...
export class OrderService {
    ...
    async getProdutos(pedidoId: string) : Promise<OrderItem[]> {
        return this.orderRepository.getProdutos(pedidoId)
    }
    getById(pedidoId: string) : Promise<Order> {
        return this.orderRepository.getById(pedidoId);
    }
    async updateStatus(pedidoId: string, status : OrderStatus) : Promise<void> {
        //regra por demanda.
        return await this.orderRepository.updateStatus(pedidoId, status)
    }
}